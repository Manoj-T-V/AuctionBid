...
const AuctionItem = () => {
  const { id } = useParams();
  const [auction, setAuction] = useState(null); // Initialize with `null` or `undefined`
  ...
  useEffect(() => {
    const fetchAuction = async () => {
      try {
        ...
        setAuction(auctionData); // Set auction data once fetched
        setBidHistory(bidData); // Assuming bid history is part of the auction data
      } catch (error) {
  ...
  if (!auction) {
    return <div>Loading...</div>; // Render a loading state if `auction` is null
  }
