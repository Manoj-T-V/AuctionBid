...
const AuctionItem = () => {
  ...
  useEffect(() => {
    const fetchAuction = async () => {
      try {
        const { data } = await axios.get(`${apiUrl}/api/bids/${id}/allbids`);
        const { auctiondata } = await axios.get(`${apiUrl}/api/auctions/${id}`);
        setAuction(auctiondata);
        setBidHistory(data); // Assuming bid history is part of the auction data
      } catch (error) {
  ...
  const handleBid = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${apiUrl}/api/bids/${id}/bid`, { amount:bid } ,
        {
          headers: {
